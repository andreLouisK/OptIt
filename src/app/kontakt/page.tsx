...
export default function KontaktPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!selectedService) return alert("Vennligst velg en tjeneste");
    
    setPending(true);
    setTimeout(() => {
      setPending(false);
      setMessage("Takk for din melding! Vi kontakter deg snart.");
    }, 1500);
  }
              ...
              <div className="space-y-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <input required className="w-full rounded-xl border border-white/10 bg-slate-800/50 px-4 py-3 text-white focus:border-sky-500 focus:outline-none transition-all outline-none" placeholder="Ola Nordmann" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-slate-400 ml-1">E-post*</label>
                    <input required type="email" className="w-full rounded-xl border border-white/10 bg-slate-800/50 px-4 py-3 text-white focus:border-sky-500 focus:outline-none transition-all outline-none" placeholder="ola@bedrift.no" />
                  </div>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <input type="tel" className="w-full rounded-xl border border-white/10 bg-slate-800/50 px-4 py-3 text-white focus:border-sky-500 focus:outline-none transition-all outline-none" placeholder="+47 000 00 000" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-slate-400 ml-1">Bedrift</label>
                    <input className="w-full rounded-xl border border-white/10 bg-slate-800/50 px-4 py-3 text-white focus:border-sky-500 focus:outline-none transition-all outline-none" placeholder="Bedrift AS" />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea required rows={4} className="w-full rounded-xl border border-white/10 bg-slate-800/50 px-4 py-3 text-white focus:border-sky-500 focus:outline-none transition-all outline-none" placeholder="Hva kan vi hjelpe deg med?" />
                </div>
